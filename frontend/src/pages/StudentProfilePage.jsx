  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 select-none">
      <div className="border-b border-slate-200 pb-4">
        <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">{t('myStudentProfileTitle', 'My Student Profile')}</h1>
        <p className="mt-1 text-sm text-slate-500 font-medium">{t('myStudentProfileSubtitle', 'View your class enrollment and account details.')}</p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="card space-y-5 md:col-span-1 border-t-4 border-emerald-600 text-center">
            ...
            <div className="border-t border-slate-100 pt-3 text-xs space-y-2 text-slate-600 text-left">
              <div><strong>{t('studentId', 'Student ID')}:</strong> {profile.uniqueId}</div>
              <div><strong>{t('class', 'Class')}:</strong> {profile.class?.name || 'N/A'}</div>
              <div><strong>{t('stream', 'Stream')}:</strong> {profile.stream?.name || 'N/A'}</div>
            </div>
          ...
          <div className="space-y-6 md:col-span-2">
            <form onSubmit={handleUpdate} className="card space-y-6">
              <h3 className="text-lg font-bold text-slate-900 border-b border-slate-100 pb-3">{t('accountDetails', 'Account Details')}</h3>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="input-label font-bold text-xs text-slate-700">{t('studentName', 'Student Name')}</label>
                  <input type="text" value={profile.user?.name || ''} disabled className="input-field bg-slate-100 cursor-not-allowed mt-1" />
                </div>
                <div>
                  <label className="input-label font-bold text-xs text-slate-700">{t('studentUniqueId', 'Student Unique ID')}</label>
                  <input type="text" value={profile.uniqueId || ''} disabled className="input-field bg-slate-100 cursor-not-allowed font-mono mt-1" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="input-label font-bold text-xs text-slate-700">{t('assignedClassDivision', 'Assigned Class / Division')}</label>
                  <input type="text" value={profile.class?.name || ''} disabled className="input-field bg-slate-100 cursor-not-allowed mt-1" />
                </div>
                <div>
                  <label className="input-label font-bold text-xs text-slate-700">{t('academicStream', 'Academic Stream')}</label>
                  <input type="text" value={profile.stream?.name || ''} disabled className="input-field bg-slate-100 cursor-not-allowed mt-1" />
                </div>
              </div>

              <div>
                <label className="input-label font-bold text-xs text-slate-700">{t('contactPhoneNumber', 'Contact Phone Number')}</label>
                <input
                  type="text"
                  value={contact}
              ...
              <button type="submit" disabled={saving} className="btn-primary">
                {saving ? 'Saving...' : t('updateContactInfo', 'Update Contact Info')}
              </button>
            </form>
