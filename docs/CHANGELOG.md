...
- **Enforced Post-Exam Publication Constraint (`student.controller.js`)**:
  - Implemented `isTestOver(qp)` evaluation helper. Question papers generated by teachers remain hidden from the student section (`/student/qp`) until the scheduled test duration is completed.
  - Direct API access to `/api/student/qp/:id` before the test schedule ends is blocked with HTTP `403 Forbidden` (`Test Schedule In Progress`).
- **Student Question Paper View & PDF Export Facilities (`StudentDashboard.jsx`, `PaperViewerPage.jsx`, `student.routes.js`)**:
  - Added **`👁️ View Paper`** and **`📥 Download PDF`** buttons to the student dashboard (`StudentDashboard.jsx`) for completed test papers.
  - Created `/api/student/qp/:id/export` endpoint streaming student-version question paper PDFs.
  - Rendered a informative `🔒 Test Schedule In Progress` alert card in `PaperViewerPage.jsx` when a student accesses a paper before the exam schedule completes.
